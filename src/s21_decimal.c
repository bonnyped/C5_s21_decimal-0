...
void clear_sys_bits(s21_decimal *num) { num->bits[3] &= SYSBITS_CORRECT_MASK; }

int get_sign(s21_decimal value) { return value.bits[3] >= 0 ? 1 : -1; }
...
int s21_is_less_or_equal(s21_decimal one, s21_decimal two) {
  ...
  res = s21_is_greater(one, two) ? 0 : 1;
...
int s21_is_greater(s21_decimal one, s21_decimal two) {
  ...
  res = s21_is_less(one, two) ? 0 : (s21_is_equal(one, two) ? 0 : 1);
...
int s21_floor(s21_decimal value, s21_decimal *result) {
  ...
  if (scale != 0) {
    if (sign == 1) {
      ...
      res += s21_truncate(value, &copy);
      res += s21_sub(copy, one, result);
    }
...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if (src != src || src == 1.0 / 0.0 || src == -1.0 / 0.0) {
    ...
    if (int_scale > 28 || int_scale < -28 ||
      ...
      while (int_scale < 0) {
        s21_div(copy, ten, &copy);
        int_scale++;
      }

      while (int_scale > 0) {
        s21_mul(copy, ten, &copy);
        int_scale--;
      }
